<template>
  <div class="container" @dragover.prevent @drop="handle_drop">
    <el-dialog
      title="添加链路"
      v-if="node_onClick"
    ...
    >
      ...
      <template v-for="(item, index) in device_graph">
        ...
        <template v-for="(paths, j) in item.down_path">
          <bm-polyline
            v-if="path_show"
            :key="paths.key"
            :path="paths.path"
...
export default {
  ...
  methods: {
    ...
    handleDelConnect(args) {
      ...
      for (
        let k = 0;
        k < this.device_graph[this.getIndexByMapID(i)].down_path.length;
        k++
      ) {
        if (
          this.device_graph[this.getIndexByMapID(i)].down_path[k].mapID === j
        ) {
    ...
    generateGraphBySuggest(suggestLocation) {
      for (let i = 0; i < suggestLocation.length; i++) {
        ...
        node.device_name = suggestLocation[i].device_name;
        node.device_type = suggestLocation[i].device_type;
        node.radiation_radius = suggestLocation[i].radiation_radius;
    ...
    handle_drop(e) {
      ...
      this.addNode({
        ...
        radiation_radius: result.radiation_radius,
        position: position,
        show_window: false,
        icon: {
          url: result.picurl,
