    ...
    <el-row :gutter="20">
      ...
      <el-col :span="4">
        <div id="positions" style="overflow: auto">
          <div id="selectNum">
            <li
              v-for="(item, index) in local_list"
              :key="index"
              class="housingList"
              @click="center(index)"
            >
              ...
              <div class="loc-num">
                <span class="pos-tag num">地点：</span
                ><span class="pos-content num">{{ item.loc_id }}</span>
              </div>
              <!-- <span>id：{{ item.id }}</span><br> -->
              <div id="loc-content">
                <span class="pos-tag">位置：</span
                ><span class="pos-content">{{ item.add }}</span
                ><br />
                <span class="pos-tag">经度：</span
                ><span class="pos-content">{{ item.loclng }}</span
                ><br />
                <span class="pos-tag">纬度：</span
                ><span class="pos-content">{{ item.loclat }}</span
                ><br />
...
export default {
  ...
  methods: {
    init() {
      ...
      geolocation.getCurrentPosition(
        r => {
          if (r.point) {
            ...
            let circle = new BMapGL.Circle(r.point, 0.1, {
            ...
            // 创建文本标注对象
            let label = new window.BMapGL.Label(name, opts);
            //文本标注样式
            label.setStyle({
              color: "#000000",
              //fontSize:'14px',
            ...
            var objloc2 = new loc(
              markers.id,
              len,
              r.address.province + r.address.city,
              r.point.lng,
              r.point.lat,
              0
            );
      ...
      function setPlace() {
        function myFun() {
          ...
          //console.log(maker);
          let circle = new BMapGL.Circle(th.userlocation, 0.1, {
            strokeColor: "#5E87DB", // 边线颜色
            fillColor: "#5E87DB", // 填充颜色。当参数为空时，圆形没有填充颜色
            strokeWeight: 2, // 边线宽度，以像素为单位
            strokeOpacity: 1, // 边线透明度，取值范围0-1
    ...
    markercomplete(event, overlay) {
      ...
      // 根据坐标得到地址描述
      myGeo.getLocation(
        new BMapGL.Point(overlay.latLng.lng, overlay.latLng.lat),
        function(result) {
          if (result) {
    ...
    delete_housing(index) {
      ...
      for (let i = 0; i < allOverlay.length; i++) {
        ...
        if (allOverlay[i].id == location.id) {
          //删除连续三个标注
