<template>
  <el-container>
    <el-header>
      ...
      <el-button v-show="deviceForm.displayform_index === 0" style="width:7%; font-size:10px; display:inline-block;"
        type="primary" @click="switch_displayform()">
        切换列表形式
      </el-button>
      <el-button v-show="deviceForm.displayform_index === 1" style="width:7%; font-size:10px; display:inline-block;"
        type="primary" @click="switch_displayform()">
        切换图片形式
      </el-button>
    </el-header>
    ...
    <el-main v-show="deviceForm.displayform_index === 0">
      <el-tabs type="card" @tab-click="device_tab_click">
        <el-tab-pane v-for="(item, index) of deviceForm.device_data.classification" :key="index" :label="item" :name="item">
          <div v-for="(deviceitem, itemid) of divide_data_class(item).slice(
              (deviceForm.graph_form.pages[index].currentpage - 1) *
                deviceForm.graph_form.pages[index].pagesize,
              deviceForm.graph_form.pages[index].currentpage *
                deviceForm.graph_form.pages[index].pagesize
            )"
            :key="itemid" style="width:1200px;
                            display:flex;">
            <el-image style="width: 600px;
                                height: 600px" :src="deviceitem.picurl">
            </el-image>
            <div style="width: 600px">
              ...
              <h5>技术参数：</h5>
              <el-table border stripe :data="deviceitem.params" style="width: 600px;
                            font-size:small;">
                <el-table-column prop="p_title" label="属性名" width="180">
                </el-table-column>
                <el-table-column prop="p_value" label="属性值" width="400">
                </el-table-column>
              </el-table>
              <h5>部署数量：</h5>
              <el-input-number v-model="deviceitem.active_num" :min="0" :max="deviceitem.repository_num" label="数量">
              </el-input-number>
            </div>
          </div>
          <el-pagination background @current-change="handleCurrentChange_graph" :current-page="deviceForm.graph_form.pages[index].currentpage"
            :page-size="deviceForm.graph_form.pages[index].pagesize" layout="total,prev,pager,next" :total="divide_data_class(item).length">
          </el-pagination>
        </el-tab-pane>
      </el-tabs>
    </el-main>
    ...
    <el-main v-show="deviceForm.displayform_index === 1">
      <el-table border stripe :data="
        ...
        "
        style="width: 1500px;
                    font-size:small;" @filter-change="filterChange">
        <el-table-column prop="name" label="设备名称" width="150">
        </el-table-column>
        <el-table-column :filters="deviceForm.list_form.class_filter" column-key="device_class" prop="class" label="设备分类"
          width="120">
        </el-table-column>
        <el-table-column :filters="deviceForm.list_form.type_filter" column-key="device_type" prop="devicetype" label="设备通信类型"
          width="120">
        </el-table-column>
        ...
        <el-table-column label="设备参数" width="420">
          <template slot-scope="scope">
            <div v-for="(paramitem, paramidex) of deviceForm.list_form
                .devices_onshow[
                scope.$index +
                  (deviceForm.list_form.currentpage - 1) *
                    deviceForm.list_form.pagesize
      ...
      <el-pagination background @current-change="handleCurrentChange_list" :current-page="deviceForm.list_form.currentpage"
        :page-size="deviceForm.list_form.pagesize" layout="total,prev,pager,next" :total="deviceForm.list_form.devices_onshow.length"
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      switch_displayform() {
        //切换展示形式（图表，列表）
        this.deviceForm.displayform_index =
          (this.deviceForm.displayform_index + 1) % 2;
      },
      getdevicedata() {
        //获取设备信息
        var that = this;
      ...
      divide_data(deviceclass_list, devicetype_list) {
        //按照 “设备分类”&“设备通信类型” 进行分类
        var resultlist = [];
        var midresult = [];
      ...
      divide_data_class(deviceclass) {
        //按照 “设备分类” 进行分类
        var resultlist = [];
        var temp = this.deviceForm.device_data.devices;
        if (deviceclass === null) return temp;
        for (var i = 0; i < temp.length; i++) {
          if (temp[i].class === deviceclass) resultlist.push(temp[i]);
        }
        return resultlist;
      },
      device_tab_click(tab, event) {
        //“图片形式”下，切换选项卡
        this.deviceForm.graph_form.currentindex = tab.index;
      },
      handleCurrentChange_graph(val) {
        //”图片形式“下翻页
        this.deviceForm.graph_form.pages[
          this.deviceForm.graph_form.currentindex
        ].currentpage = val;
      },
      ...
      filterChange(filters) {
        ...
        var last_deviceclass_list = this.deviceForm.list_form
          .last_deviceclass_list;
        ...
        if (filters.device_class != null) {
          ...
          this.deviceForm.list_form.last_deviceclass_list.splice(
            0,
            this.deviceForm.list_form.last_deviceclass_list.length
          );
          for (var i = 0; i < deviceclass_list.length; i++) {
            this.deviceForm.list_form.last_deviceclass_list.push(
              deviceclass_list[i]
            );
          }
        } else {
        ...
        if (filters.device_type != null) {
          ...
          this.deviceForm.list_form.last_devicetype_list.splice(
            0,
            this.deviceForm.list_form.last_devicetype_list.length
          );
          for (var i = 0; i < devicetype_list.length; i++) {
            this.deviceForm.list_form.last_devicetype_list.push(
              devicetype_list[i]
            );
          }
        } else {
        ...
        //重新获取device_onshow
        this.deviceForm.list_form.devices_onshow = this.divide_data(
          deviceclass_list,
          devicetype_list
        );
      ...
      sync_devicenum_list() {
        //将devices_onshow中的设备数量同步到devices中
        ...
        for (var i = 0; i < onshow.length; i++) {
          for (var j = 0; j < devices.length; j++) {
            if (onshow[i].name === devices[j].name) {
              //这里的name起到了主键的作用
              this.deviceForm.device_data.devices[j].active_num =
                onshow[i].active_num;
            }
          }
        }
