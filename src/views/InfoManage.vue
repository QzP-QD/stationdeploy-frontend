    ...
    <el-container>
        ...
        <div>
                    ...
                    width="210">
        			  ...
        			  <el-table-column label="操作">
        			    <template slot-scope="scope">
        			      ...
        			      <el-button
        			        size="mini"
        			        @click="editInfo(scope.row)">调整</el-button>
                    <el-button
                      size="mini"
                      type="danger"
        			...
        			:data="event"
        			  ...
        			  <el-table-column label="操作">
        			    <template slot-scope="scope">
        			      <el-button
        			        size="mini"
        			        @click="editEvent(scope.row)">调整</el-button>
        			    </template>
        			  </el-table-column>
        			</el-table>
        		</div>
              ...
              :header-cell-style="{background:'#FAFAFA'}"
                ...
                <span>地点：{{ locform.loc_id }}</span><br>
                <span>位置：{{ locform.add }}</span><br>
                <span>经度：{{ locform.loclng }}</span><br>
                <span>纬度：{{ locform.loclat }}</span><br>
                <span>半径：</span>
                <el-input v-model="locform.rad" style="width: 30%;" @keyup.native="proving" >{{ locform.rad }}</el-input><span>米</span><br>
                      <div slot="footer" class="dialog-footer">
                        <el-button @click="dialogFormVisible = false">取 消</el-button>
                        <el-button type="primary" @click="editSubmit('locform')">确 定</el-button>
                      </div>
        		</el-dialog>

            <el-dialog title="调整" :visible.sync="dialogEventFormVisible" width="30%">
            	<el-form :model="eventform" ref="eventform" label-width="100px" class="demo-ruleForm" >
            	<el-form-item label="类型">
                <el-select v-model="eventform.inputresult" placeholder="请选择类型">
                  <el-option
                ...
                <el-button @click="dialogEventFormVisible = false">取 消</el-button>
            	  <el-button type="primary" @click="editEventSubmit('eventform')">确 定</el-button>
            	</div>
            </el-dialog>
...
*/
    ...
    methods:{
      findAlllocation(){
        const _this = this
        this.axios.get('http://localhost:8080/static/mock/location.json').then(function (resp) {
          _this.location = resp.data.location
        })
      },
      findAllevent(){
        const _this = this
        this.axios.get('http://localhost:8080/static/mock/event.json').then(function (resp) {
          _this.event = resp.data.event
        })
      },
      findAllweather(){
        const _this = this
        this.axios.get('http://localhost:8080/static/mock/weatherdata.json').then(function (resp) {
...
/*      findAlldevices(){
      ...
      findAlleventlist(){
        const _this = this
        this.axios.get('http://localhost:8080/static/mock/eventlist.json').then(function (resp) {
          _this.eventlist = resp.data.eventlist
        })
      },
      findAllscalelist(){
        const _this = this
        this.axios.get('http://localhost:8080/static/mock/scalelist.json').then(function (resp) {
          _this.scalelist = resp.data.scalelist
        })
      },
      findAllloctypelist(){
        const _this = this
        this.axios.get('http://localhost:8080/static/mock/loctypelist.json').then(function (resp) {
          _this.loctypelist = resp.data.loctypelist
        })
      },
      findAllbusinesslist(){
        const _this = this
        this.axios.get('http://localhost:8080/static/mock/businesslist.json').then(function (resp) {
          _this.businesslist = resp.data.businessList
        })
      },
      findAllmissionlist(){
        const _this = this
        this.axios.get('http://localhost:8080/static/mock/missionlist.json').then(function (resp) {
          _this.missionlist = resp.data.missionList
        })
      },
      findAlldevices(){                                    //获取设备信息
          var that = this
          this.axios({
              method:'get',
              ...
              //设备（列表展示下）的要显示的数据
              that.deviceForm.list_form.devices_onshow =  response.data.device_data.devices
          })
      },
      NextStep(){
      ...
      findOneByLocId(row) {
        const _this = this
        this.axios.get('http://localhost:8080/static/mock/location.json').then(function(resp) {//'http://localhost:8181/location/findOne?locid=' + row.loc_id
      ...
      editSubmit(formName) {
        ...
        this.$refs[formName].validate((valid) => {
          if (valid) {
              this.axios.post('http://localhost:8181/location/edit', this.locform).then(function(resp) {
      ...
      DeleteInfo(row) {
        ...
        this.$confirm('确定删除 ?', '提示', {
          ...
          this.axios.get('http://localhost:8181/location/delete?locid=' + row.loc_id).then(function(resp) {
            if (resp.data.success) {
              _this.$message({
                type: 'info',
                message: '删除成功'
...
/*        if(_this.eventform.inputtype=="事件类型")
      ...
      findOneByInputtype(row) {
        const _this = this
        if(row.inputtype=="事件类型")
          _this.inputresultList = _this.eventlist
        if(row.inputtype=="事件规模")
          _this.inputresultList = _this.scalelist
        ...
        if(row.inputtype=="数据任务")
          _this.inputresultList = _this.businesslist
        this.axios.get('http://localhost:8080/static/mock/event.json').then(function(resp) {//'http://localhost:8181/event/findOne?event=' + row.inputtype
      ...
      editEventSubmit(formName) {
        ...
        this.$refs[formName].validate((valid) => {
          if (valid) {
              this.axios.post('http://localhost:8181/event/edit', this.eventform).then(function(resp) {
              if (resp.data.success) {
                _this.$message({
                  type: 'info',
                  message: '修改成功'
